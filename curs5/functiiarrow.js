...
console.log(toate(numere, numar => numar >= 18 && numar < 65));

// toate valorile radicalului numerelor sunt mai mici decât o valoare dată 
console.log(toate(numere, numar => Math.sqrt(numar) <= MAX));
...
console.log(există(numere, numar => numar >= MIN_ADULT
  && numar <= MAX_ADULT));
...
const truncSir = (sir, numar) =>
  // dacă lungimea șirului e mai mare de MIN caractere
  sir.length > numar ?
    // înlocuim caracterele în surplus cu șirul '...'
    sir.slice(0, numar > MIN ? numar - MIN : numar) + '...' : sir;

console.log(truncSir('Tuxy Pinguinnesscool', 7));

/* 7 */
// convertirea unui număr într-un tablou de cifre
const digitizare = numar => [...`${numar}`].map(i => parseInt(i));
